...
@section('content')
    <div class="row d-flex justify-content-center">
        <div class="col-10">
            <form action="{{ route('member.video.store', $course->slug) }}" method="POST" id="video-form">
                @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                <label for="episode">
                                    Episode
                                    <span class="badge badge-primary ml-1">Saran: Eps {{ $nextEpisode }}</span>
                                </label>
                                <input
                                    type="number"
                                    id="episode"
                                ...
                                <div id="episode-warning" class="text-danger small mt-1" style="display:none;">
                                    ...
                                    Episode ini sudah ada! Hapus atau edit episode yang lama dulu.
                                </div>
                            </div>

                            {{-- List episode yang sudah ada --}}
                            @if(count($usedEpisodes) > 0)
                            <div class="form-group">
                                <label class="text-muted small">Episode yang sudah ada (tidak bisa duplikat):</label>
                                <div>
                                    @foreach($usedEpisodes as $ep)
                                        <span class="badge badge-dark mr-1 mb-1" style="font-size:0.8rem;" title="Episode {{ $ep }} sudah ada">
                            ...
                            @else
                            <div class="form-group">
                                <small class="text-muted"><i class="fas fa-info-circle mr-1"></i>Belum ada episode. Mulai dari Ep 1!</small>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <textarea id="teori" name="teori" class="form-control @error('teori') is-invalid @enderror">{{ old('teori') }}</textarea>
                                @error('teori')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
